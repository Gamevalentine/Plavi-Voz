...
func _ready() -> void:
	done = bool(GameState.get_flag("car_seen", false))
...
func _on_body_entered(body: Node3D) -> void:
	if not body.is_in_group("player") or done:
		return
	InteractionManager.register_interactable(self, "Inspect car")
...
func is_interaction_available() -> bool:
	return not done and MissionManager.is_current("inspect_car")
...
func _on_dialogue_ended() -> void:
	...
	GameState.set_flag("car_seen", true)
	MissionManager.complete_mission("inspect_car")
